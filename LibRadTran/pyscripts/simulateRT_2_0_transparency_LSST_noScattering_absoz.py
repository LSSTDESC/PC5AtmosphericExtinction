...
# On my Linux server
libradtranpath='/exp/jemeuso/soft/radtran/libRadtran-2.0/'
...
TOPDIR='../simulations/RT/2.0/LS'  # for local tests
...
#               uvspec.inp["verbose"] = ''
                ...
                libradtranexepath=os.path.join(libradtranpath,'bin')
                uvspec.write_input(inp)
                uvspec.run(inp,out,verbose,path=libradtranpath)
                #uvspec.run(inp,out,verbose,path=libradtranexepath)
